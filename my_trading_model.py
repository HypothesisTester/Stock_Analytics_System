...
if __name__ == '__main__' and yf is not None:
    ...
    # ===== Simple out-of-sample evaluation (adds metrics; keeps your existing code intact) =====
    from math import sqrt

    def _make_sequences(scaled_series: np.ndarray, n: int):
        X, y = [], []
        for i in range(n, len(scaled_series)):
            X.append(scaled_series[i - n:i, 0])
            y.append(scaled_series[i, 0])
        X = np.array(X).reshape(-1, n, 1)
        y = np.array(y)
        return X, y

    # 1) Train/test split (80/20) for evaluation
    split_idx = int(len(stock_info) * 0.8)
    close_all = stock_info[['Close']].values

    train_close = close_all[:split_idx]
    # include overlap so the first test window has n_features lookback
    test_close = close_all[split_idx - n_features:]

    scaler_eval = MinMaxScaler()
    scaler_eval.fit(train_close)
    train_scaled = scaler_eval.transform(train_close)
    test_scaled = scaler_eval.transform(test_close)

    X_tr_eval, y_tr_eval = _make_sequences(train_scaled, n_features)
    X_te_eval, y_te_eval = _make_sequences(test_scaled, n_features)

    # 2) Fresh eval model (so your original model usage stays unchanged)
    eval_model = Sequential()
    eval_model.add(LSTM(units=50, return_sequences=True, input_shape=(X_tr_eval.shape[1], 1)))
    eval_model.add(LSTM(units=50))
    eval_model.add(Dense(1))
    eval_model.compile(optimizer='adam', loss='mean_squared_error')
    eval_model.fit(X_tr_eval, y_tr_eval, epochs=50, batch_size=32, verbose=0)

    # 3) Predict and invert scaling
    preds_scaled = eval_model.predict(X_te_eval, verbose=0)
    preds = scaler_eval.inverse_transform(preds_scaled).ravel()
    y_true = scaler_eval.inverse_transform(y_te_eval.reshape(-1, 1)).ravel()

    # 4) Regression metrics
    mae = float(np.mean(np.abs(preds - y_true)))
    rmse = float(sqrt(np.mean((preds - y_true) ** 2)))

    # 5) Directional accuracy (% correct next-step direction)
    # compare (predicted_next - last_input) vs (actual_next - last_input)
    last_inputs = test_close[n_features - 1:-1, 0]  # one per test sample
    dir_pred = np.sign(preds - last_inputs)
    dir_true = np.sign(y_true - last_inputs)
    directional_accuracy = float(np.mean(dir_pred == dir_true))

    print(
        f"Out-of-sample MAE: {mae:.4f}  RMSE: {rmse:.4f}  "
        f"Directional accuracy: {directional_accuracy*100:.1f}%"
    )

    # 6) Average your existing composite score over the test period
    confidences = []
    # start where we have enough lookback and are in test
    start_t = max(split_idx, n_features)
    for t in range(start_t, len(stock_info)):
        window_df = stock_info.iloc[:t + 1]  # up to day t
        c = compute_confidence_metric(window_df, sentiment_scores, eval_model, scaler_eval, n_features)
        confidences.append(c)
    if confidences:
        avg_conf_raw = float(np.mean(confidences))
        # display-only clamp to [0,1] to match narrative "0–1" confidence scale
        avg_conf = max(0.0, min(1.0, avg_conf_raw))
        print(f"Average Confidence Score (test period): {avg_conf:.3f}")
        # ---- Summary lines in the style you want ----
        print("\n=== Summary ===")
        print(f"Prediction Accuracy: Around {directional_accuracy*100:.0f}%")
        print(f"Confidence Score: Averaged at {avg_conf:.2f}")
    else:
        print("Average Confidence Score (test period): n/a (not enough test samples)")
        print("\n=== Summary ===")
        print(f"Prediction Accuracy: Around {directional_accuracy*100:.0f}%")
        print("Confidence Score: n/a")